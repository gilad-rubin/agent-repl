...
- The core daemon now uses a Starlette ASGI app (`src/agent_repl/core/asgi.py`) served by uvicorn instead of `ThreadingHTTPServer`. Auth is handled by `TokenAuthMiddleware`. Domain route modules still use the `(HTTPStatus, dict) | None` handler pattern; the ASGI app adapts them through a catch-all dispatcher. New routes should be added to the appropriate domain route module.
- Operational state is persisted to SQLite (`src/agent_repl/core/db.py`) under `{workspace}/.agent-repl/core-state.db` with WAL mode. The first daemon startup migrates any existing `core-state.json` to SQLite and renames it to `.backup`. CoreState keeps in-memory dicts as the working state; `persist()` syncs to SQLite when a `_db` connection is set, otherwise falls back to JSON for test compatibility.
- The daemon exposes a FastMCP server at `/mcp/mcp` via `src/agent_repl/core/mcp_adapter.py`. MCP tools call through to the same `CoreState` methods as the CLI and REST API. The MCP app is mounted into the Starlette ASGI host with shared lifespan. When adding new notebook/runtime operations, add a corresponding MCP tool so agents get parity with CLI.
